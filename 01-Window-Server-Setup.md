...
This section covers building a Windows Server 2022 virtual machine in VMware and configuring it as the domain controller, the central server that manages user accounts, computers, and security policies for the entire network.
...
I booted the virtual machine from the Windows Server 2022 ISO file, a disc image that contains the full installer, and the setup wizard launched to begin the installation. 
...
I allocated the virtual machine 50GB of storage, 4GB of memory, and 4 CPU cores to run the domain controller comfortably in the virtual environment.
...
The server needs a static IP address so it is always reachable at the same location on the network. If the address changed every time the server restarted, other computers would not be able to find it consistently. To set this up, I opened the network adapter settings and switched the server from an automatically assigned (DHCP) address to a fixed static one.
...
After configuring the static IP, the next step is to verify the change actually took effect. This can be confirmed using Command Prompt and the ipconfig command, which displays the current IP address assigned to the machine. Static IP was applied successfully.
...
To install Active Directory, I clicked `"Manage"` in the top right of Server Manager, then selected `"Add Roles and Features"`. This opened the `Add Roles and Features` wizard, the tool used to install Active Directory Domain Services onto the server, as shown in the screenshots below.
...
Once the installation was completed, I promoted the server to a `domain controller`. This step turns a regular server into the "brain" of the network, responsible for verifying logins, enforcing security policies, and keeping track of every user and computer in the domain.
...
While promoting the server, I set the root domain name to `dale.local`, the top-level name for the entire network. Every user, computer, and group created afterward belongs under this domain, and it is the name devices use to locate the domain controller for logins, security policies, and DNS. Using `.local` keeps the domain isolated from public DNS.
...
After the prerequisites check passed, the Active Directory Domain Services Configuration Wizard began the installation on ACDC01, applying the domain controller configuration. 
...
Once the installation finishes, the server automatically reboots to complete the promotion. As shown in this photo, the restart completed successfully and the new domain, `"dale.local,"` is now up and running.
...
With Active Directory installed, the next step is creating users and Organizational Units (OUs), which are containers used to group accounts by department or team. To do this, I opened Tools in the top right of Server Manager and selected `Active Directory Users and Computers`, the console used to create and manage user accounts, groups, and OUs, shown below.
...
I created two Organizational Units to represent different departments, `IT` and `HR`, and added `James White` to IT and `John Smith` to HR. To demonstrate both management methods, I created the HR OU and its user with PowerShell and the IT OU through the graphical interface. To create an OU in the GUI, right-click `dale.local`, go to New, and select Organizational Unit. Once created, the OU appears in the directory tree.

---
<img width="988" height="455" alt="image" src="https://github.com/user-attachments/assets/0eb8e23e-763b-423d-96fc-146c37e4824c" />

To add a new user to the OU, I right-clicked IT, went to New, and selected User. Shown below is the new user account form used to add someone to the IT Organizational Unit.
...
James White now appears inside the IT Organizational Unit, confirming the account was created successfully.
...
The HR Organizational Unit was created using PowerShell instead of the GUI, a faster approach commonly used in large organizations to handle repetitive tasks like onboarding many users at once.
---
...
I then created the HR user John Smith from the PowerShell command line. The command creates the account, sets its logon name, places it directly into the HR Organizational Unit, and enables it in a single step.
...
Back in Active Directory Users and Computers, the HR Organizational Unit and John Smith both appear in the directory tree, confirming the PowerShell commands worked and that both methods, GUI and PowerShell, produce the same result.
